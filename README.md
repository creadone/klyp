...
- Все данные остаются на компьютере.
...
2. Переместите `Klyp.app` в папку **Программы** (`/Applications`).
3. Запустите Klyp. Иконка ключа появится в строке меню; в Dock приложение не отображается.
...
- Нажмите на строку, чтобы скопировать пароль. Успешное копирование отмечается зелёной подсветкой.
- Наведите указатель на строку и откройте меню **…**, чтобы изменить или удалить запись.
- Перетаскивайте строки, чтобы изменить их порядок.
- В верхнем меню **…** можно включить запуск при входе, удалить все данные или выйти из приложения.
